...
kx=(2*pi/L)*[0:(n/2-1) -n/2:-1]; ky=kx;   %Wave Numbers for FFT
...
u(:,:,1)=tanh(sqrt(X.^2+Y.^2)).*cos(m*angle(X+i*Y)-(sqrt(X.^2+Y.^2)));   % Initial Condition – Spiral Pattern
v(:,:,1)=tanh(sqrt(X.^2+Y.^2)).*sin(m*angle(X+i*Y)-(sqrt(X.^2+Y.^2)));   % Initial Condition – Spiral Pattern
...
% Back to Real Space and Plotting
for j=1:length(t)-1
